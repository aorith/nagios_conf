...
perfdata="kbmemfree=${kbmemfree}KB;;;0; kbavail=${kbavail}KB;;;0; kbmemused=${kbmemused}KB;;;0; memused_perc=${memused_perc}%;$WARN;$CRIT;0; kbbuffers=${kbbuffers}KB;;;0; kbcached=${kbcached}KB;;;0; kbcommit=${kbcommit}KB;;;0; commit_perc=${commit_perc}%;$WARN;$CRIT;0;"
...
if _decimal_compare $memused_perc $CRIT || _decimal_compare $commit_perc $CRIT; then 
    status=2
    text="CRITICAL: ${text}"
elif _decimal_compare $memused_perc $WARN || _decimal_compare $commit_perc $WARN; then 
    status=1
    text="WARNING: ${text}"
fi
